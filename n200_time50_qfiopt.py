    ...
    def run(self):
        ...
        for epi_counter in range(num_maxloop):
            ...
            if epi_counter % 500 == 0 and self.worker_idx == 0:
                np.savez('n200_data/record_time50_n200_' + now_time.strftime('%m%d') + '.npz', recording=recording)
                np.savez('n200_data/a_r_record_time50_n200_' + now_time.strftime('%m%d') + '.npz',
                         a_r_recording=np.array(a_r_recording))
                self.server.save_weights(
                    'n200_data/saved_weights/in_{}-th_loop_weights.ckpt'.format(epi_counter))
